...
namespace LibConexionBD
{
    public class ClsConexion
    {
        ...
        private bool blnBDAbierta; //CUANDO LA CONEXION ESTE ABIERTA O CERRADA
        private string strCadenaCnx; //SE ALMACENA PARAMETRO CONEXION LE ENTREGA
        private SqlConnection objCnnBD; //ES UNA CLASE OBJETO TIPO SQLCONNECTION
        private SqlCommand objCmdBD; 
        private SqlDataReader objReader; //ES UN INTERMEDIARIO PARA LLENAR EL DATASET 
        private SqlDataAdapter dapGenerico; //Intermediario para llenar el dataset
        private DataSet dts; //CONJUNTO DE TABLAS
        ...
        private bool AbrirConexion()
            // se llama el metodo cadena de conexion
        {
            if (GenerarCadenaConexion())
            {
                try
                {
                    objCnnBD.ConnectionString = strCadenaCnx; //ConnectionString permite recibir la cadena conexion de ese LBparametro
                    objCnnBD.Open(); //PERMITE CONECTARME A LA BASE DE DATOS  
                    blnBDAbierta = true;
                    return true;
                }
                catch (Exception ex)
        ...
        {
            if (SentenciaSQL == "USP_Consultars")
            {
            ...
            if (blnCon_Parametros)
                objCmdBD.CommandType = CommandType.StoredProcedure; //StoredProcedure son procedimientos almacenados
            else
            ...
            try
            {
                objReader = objCmdBD.ExecuteReader();//Ejecuta una setencia tipo Select from solamente
                return true;
            }
            catch (Exception ex)
            {
